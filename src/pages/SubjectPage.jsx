...
export default function SubjectPage() {
  ...
  const { elapsed, start: startTimer, stop: stopTimer } = useSessionTimer()
  ...
  async function openTopicPanel(topic) {
    if (activeTopic?.id === topic.id) {
      ...
      if (sessionId) { stopTimer(); await endSession(sessionId, elapsed) }
      const sid = await startSession(subjectId, topic.id)
      setSessionId(sid)
      startTimer()
            ...
            >
              ...
              <div className="flex items-center gap-2">
                {activeTopic?.id === topic.id && (
                  <span className="text-xs text-indigo-500 font-medium">{Math.round(elapsed)}s</span>
                )}
